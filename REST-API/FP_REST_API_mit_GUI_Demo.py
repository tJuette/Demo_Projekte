...
class MeinHauptFenster(QMainWindow):
    ...
    def xml_generieren(self):
        ...
        # Ein XML-String wird erstellt, mit einer Mischung aus List-Comprehensions und Schleifen werden aus dem Dictionary mit XML-Notation 
        # die Schlüssel als Tags und die Werte als Inhalt dieser Zeichenkette hinzugefügt. So ensteht ein vollständiges XML-Dokument in String-Form. 
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>\n<bestsellers>\n'
        xml_string += ''.join([f'\t<{k}>{v}</{k}>\n' for k, v in xml_notation_dic.items() if k != 'results']) + '\t<results>\n'
        xml_string += ''.join([f'\t\t<{k}>{v}</{k}>\n' for k, v in xml_notation_dic['results'].items() if k not in ('books', 'corrections')])
        xml_string += '\t\t<books>\n'
        for book in xml_notation_dic['results']['books']:
            xml_string += ''.join([3*'\t'+'<book>\n'] + [4*'\t'+f'<{k}>{v}</{k}>\n' for k, v in book.items() if k not in ('isbns', 'buy_links', 'book_uri')])
            xml_string += 4*'\t'+'<isbns>\n'
            for isbns in book['isbns']:
                xml_string += ''.join([5*'\t'+'<i10_i13>\n'] + [6*'\t'+f'<{k}>{v}</{k}>\n' for k, v in isbns.items()] + [5*'\t'+'</i10_i13>\n'])
            xml_string += 4*'\t'+'</isbns>\n'+4*'\t'+'<buy_links>\n'
            for isbns in book['buy_links']:
                xml_string += ''.join([5*'\t'+'<store>\n'] + [6*'\t'+f'<{k}>{v}</{k}>\n' for k, v in isbns.items()] + [5*'\t'+'</store>\n'])
            xml_string += 4*'\t'+'</buy_links>\n' + ''.join([4*'\t'+f'<{k}>{v}</{k}>\n' for k, v in book.items() if k == 'book_uri'] + [3*'\t'+'</book>\n'])
        xml_string += '\t\t</books>\n'
        xml_string += ''.join(['\t\t<corrections>\n'] + [3*'\t'+f"{i}\n" for i in xml_notation_dic['results']['corrections']] + ['\t\t</corrections>\n']) 
        xml_string += '\t</results>\n</bestsellers>'
