...
class Waehrungsrechner(QMainWindow):
    def __init__(self):
        ...
        try:
            json_string = urllib.request.urlopen("https://api.exchangerate.host/latest").read().decode("utf-8")
            self.raten = {"EUR":json.loads(json_string)}
        except BaseException as err:
            QMessageBox.about(self, "Fehler", f"{err}")
            app.quit()
        
        self.waehrungen = {"Euro":"EUR", "Kanadischer Dollar":"CAD", "Hongkong-Dollar":"HKD", "Isländische Krone":"ISK", "Philippinischer Peso":"PHP", 
                 "Dänische Krone":"DKK", "Ungarischer Forint":"HUF", "Tschechische Krone":"CZK", "Australischer Dollar":"AUD", "Rumänischer Leu":"RON",
                 "Schwedische Krone":"SEK", "Indonesische Rupiah":"IDR", "Indische Rupie":"INR", "Brasilianischer Real":"BRL", "Russischer Rubel":"RUB",
                 "Kroatische Kuna":"HRK", "Japanischer Yen":"JPY", "Thai Baht":"THB", "Schweizer Franken":"CHF", "Singapur-Dollar":"SGD", "Polnischer Zloty":"PLN",
                 "Bulgarischer Lew":"BGN", "Türkische Lira":"TRY", "Chinesischer Yuan":"CNY", "Norwegische Krone":"NOK", "Neuseeland-Dollar":"NZD", 
                 "Südafrikanischer Rand":"ZAR", "US-Dollar":"USD", "Mexikanischer Peso":"MXN", "Israelischer Schekel":"ILS", "Britischer Pfund":"GBP", 
                 "Südkoreanischer Won":"KRW", "Malaysischer Ringit":"MYR"}
        ...
        self.ursprungswaehrung.addItems(map(lambda x : x, sorted(self.waehrungen)))
        self.endwaehrung.addItems(map(lambda x : x, sorted(self.waehrungen)))
    ...
    def umrechnen(self):
        ...
        eingabe = "".join(filter(lambda x : x in "0123456789,", self.eingabe.text()))
        if eingabe.count(",") > 1:
        ...
        if self.raten.get(basis):
            ...
            try:
                json_string = urllib.request.urlopen(f"https://api.exchangerate.host/latest?base={basis}").read().decode("utf-8")
