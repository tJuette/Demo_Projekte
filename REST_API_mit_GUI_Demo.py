...
class MeinHauptFenster(QMainWindow):
    def __init__(self): 
        ...
        self.feld_json_speichern = QLineEdit(eingabe_container)
        self.feld_json_speichern.setFixedWidth(300)
        self.feld_json_speichern.setPlaceholderText("Pfad eingeben oder im Benutzerverzeichnis speichern")
        ...
        eingabe_gitter.addWidget(self.feld_json_speichern, 3, 0, QtCore.Qt.AlignCenter)
        ...
        self.feld_xml_speichern = QLineEdit(eingabe_container)
        self.feld_xml_speichern.setFixedWidth(300)
        self.feld_xml_speichern.setPlaceholderText("Pfad eingeben oder im Benutzerverzeichnis speichern")
        ...
        eingabe_gitter.addWidget(self.feld_xml_speichern, 3, 1, QtCore.Qt.AlignCenter)
    ...
    def json_speichern(self):
        if not self.json_zeichenkette_roh:
            ...
            return
        try:
            if self.feld_json_speichern.text():
                pfad = pathlib.Path(self.feld_json_speichern.text())
                if not pfad.drive or not pfad.name or pfad.suffix != ".json":
                    QMessageBox.about(self, "Fehler", 'Sie müssen zumindest einen Laufwerksbuchstaben mit Doppelpunkt und einem oder zwei Schrägstrichen bzw. Backslashes angeben, sowie einen Dateinamen mit der Endung ".json"!')
                    return
                pfad.parent.mkdir(exist_ok=True, parents=True)
            else:
                pfad = pathlib.Path().home().joinpath("NY Bestseller.json")
            bestaetigung = QMessageBox()
            bestaetigung.setIcon(QMessageBox.Question)
            bestaetigung.setWindowTitle("JSON-Dokument speichern")
            bestaetigung.setText(f"Das JSON-Dokument im Pfad {pfad} speichern?")
    ...
    def xml_generieren(self):
        ...
        # Ein XML-String wird erstellt, Schleifen durchlaufen das Dictionary mit XML-Notation immer bis zum nächsten
        # Dictionary oder zur nächsten Liste und fügen die Schlüssel als Tags und die Werte als Texte hinzu. So ensteht 
        # Stück für Stück ein vollständiges XML-Dokument als String. 
        xml_string = '<?xml version="1.0" encoding="UTF-8"?>\n<bestsellers>\n'
        for k,v in xml_notation_dic.items():
            if k == "results":
                break
            xml_string += f"    <{k}>{v}</{k}>\n"
        xml_string += "    <results>\n"
        for k,v in xml_notation_dic['results'].items():
            if k == "books":
                 break
            xml_string += f"        <{k}>{v}</{k}>\n"
        xml_string += "        <books>\n"
        for book in xml_notation_dic['results']['books']:
            ...
            for isbns in book['isbns']:    
                xml_string += "                    <i10_i13>\n"
                for k, v in isbns.items():
                    xml_string += f"                        <{k}>{v}</{k}>\n"
                xml_string += "                    </i10_i13>\n"
            xml_string += "                </isbns>\n"
            xml_string += "                <buy_links>\n"
            for links in book['buy_links']:
                xml_string += "                    <store>\n"
                for k, v in links.items():
                    xml_string += f"                        <{k}>{v}</{k}>\n"
                xml_string += "                    </store>\n"
            xml_string += "                </buy_links>\n"
            xml_string += f"                <book_uri>{book['book_uri']}</book_uri>\n"
            xml_string += "            </book>\n"
        xml_string += "        </books>\n"     
    ...
    def xml_anzeigen(self):
        if not self.xml_zeichenkette_formatiert:
            ...
            return
        self.ausgabefeld.setPlainText(self.xml_zeichenkette_formatiert)            
    ...
    def xml_speichern(self):
        if not self.xml_zeichenkette_formatiert:
            ...
            return
        try:
            if self.feld_xml_speichern.text():
                pfad = pathlib.Path(self.feld_xml_speichern.text())
                if not pfad.drive or not pfad.name or pfad.suffix != ".xml":
                    QMessageBox.about(self, "Fehler", 'Sie müssen zumindest einen Laufwerksbuchstaben mit Doppelpunkt und einem oder zwei Schrägstrichen bzw. Backslashes angeben, sowie einen Dateinamen mit der Endung ".xml"!')
                    return
                pfad.parent.mkdir(exist_ok=True, parents=True)
            else:    
                pfad = pathlib.Path().home().joinpath("NY Bestseller.xml")
            bestaetigung = QMessageBox()
            bestaetigung.setIcon(QMessageBox.Question)
            bestaetigung.setWindowTitle("XML-Dokument speichern")
            bestaetigung.setText(f"Das XML-Dokument im Pfad {pfad} speichern?")
            bestaetigung.setStandardButtons(QMessageBox.Ok | QMessageBox.Cancel)
            if bestaetigung.exec() == QMessageBox.Cancel:
                return
